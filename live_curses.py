...
def trigger_clip_record(live):
    song = live.get_document()
    track = song.tracks[0]
    # track.clip_slots[0].create_clip(8)
    triggered = True
    track.clip_slots[0].fire(record_length=8)
...
def process_keyboard(scr, live):
    ...
    if key == ' ':
        ...
        create_midi_track(live)
    elif key == 't':
        trigger_clip_record(live)
    elif key == 'q':
...
def update_status(scr, live):
    ...
    tempo = song.tempo
    level = song.tracks[0].output_meter_level
    status = 'playing' if song.is_playing else 'stopped'
    text = 'Song %s at %s.%s.%s, tempo: %s, level: %s\nTAP: %s' % (
        status, time.bars, time.beats, time.sub_division, tempo, level, taps.get_tap())
    if triggered and song.tracks[0].clip_slots[0].is_playing != 2:
        taps.stop()
...
def print_instructions(scr):
    ...
    scr.addstr(4, 1, 'A = TRACK')
    scr.addstr(4, 1, 't = trigger clip')
...
def main_gui(args, scr):
    ...
    song.tracks[0].add_output_meter_level_listener(updater)
    song.tracks[0].arm = 1
    updater()
    ...
    try:
        ...
        song.tracks[0].remove_output_meter_level_listener(updater)
        song.tracks[0].clip_slots[0].remove_is_playing_listener(updater)
        conn.close()
